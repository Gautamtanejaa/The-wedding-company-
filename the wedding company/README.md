...
### Architecture (SVM-style)

```text
title Organization Management Service - Architecture
direction right

Client [icon: monitor, color: blue]
API [icon: server, color: green]
Master DB [icon: database, color: orange]
Org Collections [icon: database, color: purple]

// Master DB holds global metadata
note over Master DB
  Contains:
  - organizations
  - admins
...
Client -> API: HTTPS / REST requests
API -> Master DB: Org & admin metadata
API -> Org Collections: Per-org data (org_<slug>)
```

### Main Sequence Flow (SVM-style)

```text
autoNumber nested
title Multi-Tenant Organization Management - Main Flow

Client [icon: monitor, color: blue]
API [icon: server, color: green]
Master DB [icon: database, color: orange]
Org Collection [icon: database, color: purple]

// 1) Organization creation
Client > API: POST /org/create (org_name, email, password)
API > Master DB: Check org & admin uniqueness
Master DB --> API: OK

API > Master DB: Insert admin + organization
API > Master DB: Create collection org_<slug>
API --> Client: 201 Created (org_id, admin_id, collection_name)

// 2) Admin login
Client > API: POST /admin/login (email, password)
API > Master DB: Find admin by email
Master DB --> API: Admin + org_id
API > API: Verify password

alt Password valid
  API > Master DB: Load organization
  Master DB --> API: Org info
  API > API: Generate JWT (admin_id, org_id, org_name)
  API --> Client: 200 OK (JWT)
else Invalid credentials
  API --> Client: 401 Unauthorized
end

// 3) Protected operation (example: update org)
Client > API: PUT /org/update (Bearer JWT, new data)
API > API: Validate JWT
API > Master DB: Resolve admin + organization
Master DB --> API: Admin + org
API > Org Collection: Update org-specific data
API --> Client: 200 OK (updated)

// 4) Optional: delete organization
opt Organization deletion
  Client > API: DELETE /org/delete (Bearer JWT)
  API > Master DB: Delete admin + organization
  API > Master DB: Drop org_<slug> collection
  API --> Client: 200 OK (organization deleted)
end
```
